...
hours_str = timeClient.getHours(), mins_str = timeClient.getMinutes(), and secs_str = timeClient.getSeconds() is used to get the time variables for the Timer. 
But the time that we get from them are in String format so we must first convert them into uint8_t data type, which is done by stringToUint() function.
